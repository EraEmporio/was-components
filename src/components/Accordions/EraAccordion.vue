    ...
    <template v-for="(item, index) in items" :key="index">
      <AccordionItem class="AccordionItem" :value="index.toString()">
        <AccordionHeader class="flex">
          <AccordionTrigger
            :class="twMerge('AccordionTrigger h-11 flex flex-1 py-0 px-5 items-center justify-between bg-mercury-50 hover:bg-mercury-100', style.trigger)"
          >
            <span>{{ item.title }}</span>
            <Icon
              icon="radix-icons:chevron-down"
              :class="twMerge('AccordionChevron text-mercury-700', style.chevron)"
              aria-hidden
            />
          </AccordionTrigger>
        </AccordionHeader>
